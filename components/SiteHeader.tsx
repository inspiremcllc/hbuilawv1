...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/96 shadow-sm backdrop-blur">
      ...
      <div className="container-site flex h-24 items-center justify-between gap-6">
        ...
        <nav aria-label="Primary navigation" className="hidden items-center gap-7 lg:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-medium text-[var(--navy)] transition hover:text-[var(--gold-dark)] focus-visible:outline focus-visible:outline-2"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <a
            href={phoneHref}
            className="inline-flex size-11 items-center justify-center rounded-[8px] border border-[var(--line)] text-[var(--navy)] transition hover:border-[var(--gold)] focus-visible:outline focus-visible:outline-2"
        ...
        <button
          type="button"
          className="inline-flex size-11 items-center justify-center rounded-[8px] border border-[var(--line)] text-[var(--navy)] lg:hidden"
          aria-expanded={open}
